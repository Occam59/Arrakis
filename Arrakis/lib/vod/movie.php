...
class Movie
{
    ...
    public function set_advert_urls($preplay_url, $postplay_url)
    {
        $this->preplay_url = isset($preplay_url) ?
            strval($preplay_url) : null;
        $this->postplay_url = isset($postplay_url) ?
            strval($postplay_url) : null;
    }
    ...
    public function get_postplay_vod_info()
    {
        if (!isset($this->postplay_url))
            return null;

        $series_array[] = array(
            PluginVodSeriesInfo::name => "series1",
            PluginVodSeriesInfo::playback_url => $this->postplay_url,
            PluginVodSeriesInfo::playback_url_is_stream_url => true,
        );

        return array(
            PluginVodInfo::name => "series1",
    ...
    public function get_main_vod_info($sel_id, $buffering_ms)
    {
        ...
        $actions[GUI_EVENT_PLAYBACK_STOP] =
            ActionFactory::vod_play(
                $this->get_postplay_vod_info());
        ...
        return array(
            ...
            PluginVodInfo::actions => $actions,
            PluginVodInfo::initial_position_ms => 0,
        );
    }

    public function get_preplay_vod_info($sel_id, $buffering_ms)
    {
        if (!isset($this->preplay_url))
            return null;

        $series_array[] = array(
            PluginVodSeriesInfo::name => "series1",
            PluginVodSeriesInfo::playback_url => $this->preplay_url,
            PluginVodSeriesInfo::playback_url_is_stream_url => true,
        );

        $actions[GUI_EVENT_PLAYBACK_STOP] =
            ActionFactory::vod_play(
                $this->get_main_vod_info($sel_id, $buffering_ms));
    ...
    {
        $info = $this->get_preplay_vod_info($sel_id, $buffering_ms);
        if (isset($info))
            return $info;

        return $this->get_main_vod_info($sel_id, $buffering_ms);
    }
}
